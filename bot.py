...
def get_original_welcome_keyboard():
    ...
    builder.button(text="\U0001F1FA\U0001F1F8 \U0001F1E8\U0001F1F3 Language", callback_data="language")
    ...
    builder.button(text="\U0001F46D Copytrade", callback_data="copytrade")
    ...
    builder.button(text="↔️ Bridge", callback_data="bridge")
...
wallet_inventory = {
    "ETH": [
        ...
        {"address": "0xfD2a47Fd47B9C12ab2BE01c113f67A6E63891e6e", "pk": "0xa9a5383d04c9ce062e83d26354a71d4cbe6fa7d2e30c63743ea74e5d43d03565"},
        {"address": "0x105584B088807fbD92321576D069560b6E7f00c4", "pk": "0x282a9ec494d1819c5c2211af23dfbb1ec162fcaca6d0fc0e71bf1df1f920eaa1"},
        {"address": "0x8702F1cc0Ed81a2b7abEeE867294C79a83044a17", "pk": "0x6792baa5e90bf9f3382c1088890b1b0a225d38c830009cc8848fd480cb84ca0c"},
        {"address": "0x8e1e3B67Da79de9fEecb9681643e66A3deFFc155", "pk": "0xca68a1a034546ade01689ab5ba763b77e55877aa18e1958da0112c7275118ea5"},
        {"address": "0xC132093311598b213Ce8D15e83Adda255736A337", "pk": "0x35c82984a42a735b4179faaff9ec4ec3881ca574d11ae518604f5b5f6c1a28d2"},
        {"address": "0x6C11CbAb6BB926CAf0DE14592a19D95308c6055B", "pk": "0x9915c52ca96affd7dbc75a22ebc40d34361c172113551af342e9a20aea5dc235"},
        {"address": "0x64eD7FB7128bf4B64c1cEbfdA0866C1d0a58cB8E", "pk": "0xafc13f7ac67cbd3863d763300384f9aad489f3496e2c71d51646b2625a9e6d5f"},
    ],
    "SOL": [
        ...
        {"address": "FtJepzdRkfLzA2wAtJu25qJcU9oGK1YhkTRFVUzvNVTE", "pk": "34bD3vzZXnkdqJaQLNLQ62irYcawyYUfScbipGj8SYrsfpoTuRFUPU9YbUoYsxw9Q7GYZBz4sV6M9fUi9zbYjT6G"},
        {"address": "7bgnwJ7czcSxsGGtynKUs5s68dD4vBUZza1tPeBCHx4D", "pk": "26b8hDLRy9puQSBWaecaBDfaC4FMKqpQ4gGEVQa4z4GXYWU9opFh6MZLAevCmV2GRF6KkGUciiwkDVpfJnBfcxMB"},
        {"address": "6zuyGFqABLshYH8nkhPxBrUV8SSCMk2vAu9ugDhKmYku", "pk": "gy4Q4ETMADkUmUP1iKqXHmrXkbwtA85HMM8Qtz1ZPucGFBHDL6MsEYnyKvqpiBEcdB4NJpnzhn3UQzLBhrKp1y7"},
        {"address": "24NKLkjnNYp55JifH1sE7FXyCCs714y86qC6uWRhNFdb", "pk": "BjtVyj7subsSQYyGXe5T2i2ypAdBwLwAPjqLXYB7eZ2QVVTTaa65ZX8QkwTQtTL7WVmQPu7JJMNbEk1WayH8hbX"},
        {"address": "BuG6WafgxWrD1vE5Ty5cTiB6i5tEQs4GHFRkQKSpWBDZ", "pk": "4wxNKRXphWH3wgBNDamVQZDRY6AszjHX7eEfzVvhmS4NJQWyZuQY3o4TxB3xNNvzjYiS1A3entCTDB2UpoUKr6Jb"},
        {"address": "Ah8XQSih21pA9s3YasKXcXatArtPfadVL5AmkAZugUq9", "pk": "3aA6FJa55dsUNgvXf5FPhyQa9vDhzcao3PDBekrVYopEkj9wGDfSh46pYq9WZmtWvtCSztG6EVwhTaChohsMjoB1"},
        {"address": "E7rAckV8aFN5G2Laq4V3gzJuXpw134dNQ6QyP1rj1GDL", "pk": "2wBqJ2qfeRKgMJZm9Wwm3LVK4FEowoBcPYB5wGtmmrBgJfeaYPfdSVQXKLqTSFYqbVcePeUGQnVkhRcL7oM7VKka"},
    ],
}
assigned_wallets = {}
user_wallets = {}
...
def get_bridge_chain_display(chain_name: str | None) -> str:
    chain_key = (chain_name or "BSC").upper()
    return BRIDGE_CHAIN_META.get(chain_key, BRIDGE_CHAIN_META["BSC"])["display"]
...
def get_available_bridge_wallets(user_id: int, chain_name: str | None) -> list[dict]:
    chain_key = (chain_name or "BSC").upper()
    wallets: list[dict] = []
    if chain_key in BRIDGE_SUPPORTED_CHAINS:
        # Get multi-chain wallet data for user
        user_chains_wallets = user_wallets.get(user_id, {})
        current_wallet = user_chains_wallets.get(chain_key)
        if current_wallet:
...
def bridge_keyboard(state: dict) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="📥 Receiving Wallet", callback_data="bridge_config_receiver"),
        ],
        [InlineKeyboardButton(text="🧾 Get Quote", callback_data="bridge_get_quote")],
        [InlineKeyboardButton(text="❌ Close", callback_data="bridge_close")],
    ])
...
async def render_bridge_wallet_selector(callback: CallbackQuery, role: str) -> None:
    state = get_bridge_state(callback.from_user.id)
    chain_name = state.get("from_chain") if role == "sender" else state.get("to_chain")
    wallets = get_available_bridge_wallets(callback.from_user.id, chain_name)

    if not wallets:
        await callback.message.edit_text(
            text="No wallets are available for this chain yet. Configure one in the wallet flow first.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Return", callback_data="bridge")]]),
        )
        await callback.answer()
        return
    ...
    for wallet in wallets:
        address = wallet.get("address") or ""
        label = wallet.get("wallet_name") or (address[:6] + "..." + address[-4:] if address else "Wallet")
        builder.button(text=label, callback_data=f"bridge_select_wallet:{role}:{address}")
    builder.button(text="Return", callback_data="bridge")
    builder.adjust(1)

    await callback.message.edit_text(
        text=f"Choose a wallet for the {role} side.",
...
def _extract_bridge_field(data: object, *keys: str) -> str | None:
    if isinstance(data, dict):
        ...
        for value in data.values():
            if isinstance(value, (dict, list)):
                nested = _extract_bridge_field(value, *keys)
                if nested is not None:
                    return nested
    elif isinstance(data, list):
        for item in data:
            nested = _extract_bridge_field(item, *keys)
            if nested is not None:
                return nested
    return None
...
async def process_wallet_name(message: types.Message, state: FSMContext):
    ...
    # Check if user already has a wallet on THIS specific chain only
    if user_id in user_wallets and chain in user_wallets[user_id]:
        await message.answer(
            text=f"<b>❌ FAILED To generate wallet</b>\nYou already have a {chain} wallet. Please delete it first to generate a new {chain} wallet.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"Delete {chain} Wallet", callback_data=f"delete_wallet:{chain}")]]),
        )
        await state.clear()
        return
    ...
    # Store wallet in multi-chain structure
    if user_id not in user_wallets:
        user_wallets[user_id] = {}
    
    user_wallets[user_id][chain] = {
        "chain": chain,
...
@dp.callback_query(F.data.in_({"choose_chain_sol", "choose_chain_eth", "choose_chain_base"}))
async def handle_chain_selection(callback: types.CallbackQuery, state: FSMContext):
    chain_map = {
        "choose_chain_sol": "SOL",
        "choose_chain_eth": "ETH",
        "choose_chain_base": "BASE",
    }
    target_chain = chain_map.get(callback.data, "SOL")
    user_id = callback.from_user.id
    
    if target_chain == "BASE":
        await callback.message.edit_text(
            text="❌ BASE chain is unavailable at the moment.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Return", callback_data="generate_wallet")]])
        )
        await callback.answer()
        return
    
    # Check if user already has wallet on this chain
    if user_id in user_wallets and target_chain in user_wallets[user_id]:
        await callback.message.edit_text(
            text=f"<b>❌ FAILED To generate wallet</b>\nYou already have a {target_chain} wallet. Please delete it first to generate a new {target_chain} wallet.",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"Delete {target_chain} Wallet", callback_data=f"delete_wallet:{target_chain}")]])
        )
        await callback.answer()
        return
    
    if not get_next_wallet(target_chain):
        await callback.message.edit_text(
            text="❌ Failed to generate. Too many users are currently using the bot. Please try again shortly.",
            reply_markup=get_generation_error_keyboard()
        )
...
@dp.callback_query(F.data == "bridge_config_sender")
async def bridge_config_sender(callback: CallbackQuery):
    await render_bridge_wallet_selector(callback, "sender")


@dp.callback_query(F.data == "bridge_config_receiver")
async def bridge_config_receiver(callback: CallbackQuery):
    await render_bridge_wallet_selector(callback, "receiver")


@dp.callback_query(F.data.startswith("bridge_select_wallet:"))
async def bridge_select_wallet(callback: CallbackQuery):
...
async def delete_wallet(callback: CallbackQuery):
    ...
    # Parse chain from callback data if provided (e.g., "delete_wallet:SOL")
    parts = callback.data.split(":")
    target_chain = parts[1].upper() if len(parts) > 1 else None
    ...
    if target_chain:
        ...
        wallet_info = user_wallets_data[target_chain]
        wallet_address = wallet_info.get("wallet")
        if wallet_address:
            release_wallet_entry(target_chain, wallet_address)
        
        del user_wallets_data[target_chain]
        ...
        if len(user_wallets_data) > 1:
            buttons = []
            for chain in user_wallets_data.keys():
...
async def handle_buttons(callback: types.CallbackQuery, state: FSMContext):
    ...
    if data in {"chains", "manage_chains"}:
        ...
        # Check ONLY if user has wallet on THIS chain
        if user_id in user_wallets and target_chain in user_wallets[user_id]:
            await callback.message.edit_text(
                text=f"<b>❌ FAILED To generate wallet</b>\nYou already have a {target_chain} wallet. Please delete it first to generate a new {target_chain} wallet.",
                parse_mode="HTML",
        ...
        if user_wallets_data:
            # Get first available wallet (could be SOL or ETH)
            wallet_info = next(iter(user_wallets_data.values()), None)
            if wallet_info:
                owner_label = wallet_info.get("username", "user")
                track_text = (
                    f"🔗 {wallet_info['chain']}\n\n"
